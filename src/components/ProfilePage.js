...
class Profile extends React.Component {
  ...
  render() {
    ...
    const myLastPhotos = _.reverse(_.sortBy(myPhotos, o => o.properties.moderated)).slice(0,20);
    ...
    return (
      <PageWrapper label={label} handleClose={this.props.handleClose} header={false}>
        <div className={'profile-info'}>
          ...
          {myPhotos &&
          <Typography variant='body1' >Num. of uploads <strong>{myPhotos.length}</strong></Typography>
          }
          {!isNaN(numPieces) &&
          <Typography variant='body1' >Total Pieces <strong>{numPieces}</strong></Typography>
          }
          ...
          { myLastPhotos.length &&
          <div>
            <Typography variant='h6' className={classes.centered}>Last {myLastPhotos.length} approved</Typography>

            {_.map(myLastPhotos, photo => (
              <div className={classes.centered} key={photo.properties.id}>
                <Typography variant='body1' >
                  {photo.properties.pieces &&
                  <span><strong>{photo.properties.pieces}</strong> pieces </span>
                  }
                  <Link to={this.calcUrl(photo)} onClick={() => handlePhotoClick(photo)}>{photo.properties.moderated.toDateString()}</Link>
                </Typography>
              </div>
            ))}
